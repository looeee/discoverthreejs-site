...
export default function addJavaScriptMode( CodeMirror ) {
  ...
  CodeMirror.defineMode( 'javascript', ( config, parserConfig ) => {
    ...
    function isModifier( name ) {
      return name == 'public' || name == 'private' || name == 'protected' || name == 'abstract' || name
        == 'readonly';
    }
    ...
    function statement( type, value ) {
      ...
      if ( type == 'keyword d' ) {
        return cx.stream.match( /^\s*$/, false ) ? cont() : cont( pushlex( 'stat' ),
          maybeexpression, expect( ';' ), poplex );
      }
      ...
      if ( type == 'variable' ) {
        ...
        if ( isTS && ( value == 'module' || value == 'enum' || value == 'type' ) && cx.stream.match( /^\s*\w/,
          false ) ) {
          cx.marked = 'keyword';
      ...
      if ( type == 'catch' ) {
        return cont( pushlex( 'form' ), pushcontext, maybeCatchBinding, statement, poplex,
          popcontext );
      }
    ...
    function expressionInner( type, value, noComma ) {
      if ( cx.state.fatArrowAt == cx.stream.start ) {
        ...
        if ( type == '(' ) {
          return cont( pushcontext, pushlex( ')' ), commasep( funarg, ')' ), poplex, expect(
            '=>',
          ), body, popcontext );
        }
    ...
    function maybeTarget( noComma ) {
      return function ( type ) {
        ...
        if ( type == 'variable' && isTS ) {
          return cont( maybeTypeArgs, noComma ? maybeoperatorNoComma
            : maybeoperatorComma );
        }
    ...
    function functiondef( type, value ) {
      ...
      if ( type == '(' ) {
        return cont( pushcontext, pushlex( ')' ), commasep( funarg, ')' ), poplex, mayberettype,
    ...
    function enumdef() {
      return pass( pushlex( 'form' ), pattern, expect( '{' ), pushlex( '}' ), commasep( enummember, '}' ), poplex,
        poplex );
    }
