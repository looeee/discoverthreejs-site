...
const bidiOrdering = ( function() {
  ...
  function charType( code ) {
    ...
    if ( 0x590 <= code && code <= 0x5f4 ) {
      return "R"
    }
    if ( 0x600 <= code && code <= 0x6f9 ) {
      return arabicTypes.charAt( code - 0x600 )
    }
    if ( 0x6ee <= code && code <= 0x8ac ) {
      return "r"
    }
    if ( 0x2000 <= code && code <= 0x200b ) {
      return "w"
    } else if ( code == 0x200c ) {
      return "b"
    } else {
...
const on = function( emitter, type, f ) {
  if ( emitter.addEventListener ) {
    ...
    map$$1[ type ] = ( map$$1[ type ] || noHandlers )
      .concat( f );
  }
};
...
function off( emitter, type, f ) {
  if ( emitter.removeEventListener ) {
    ...
    const map$$1 = emitter._handlers;
    const arr = map$$1 && map$$1[ type ];
    if ( arr ) {
      const index = indexOf( arr, f );
      if ( index > -1 ) {
        map$$1[ type ] = arr.slice( 0, index )
...
function hasHandler( emitter, type ) {
  return getHandlers( emitter, type )
    .length > 0;
}
...
function hasBadBidiRects( measure ) {
  ...
  const r0 = range( txt, 0, 1 )
    .getBoundingClientRect();
  const r1 = range( txt, 1, 2 )
    .getBoundingClientRect();
...
function hasBadZoomedRects( measure ) {
  ...
  const fromRange = range( node, 0, 1 )
    .getBoundingClientRect();
...
StringStream.prototype.column = function() {
  if ( this.lastColumnPos < this.start ) {
    this.lastColumnValue = countColumn( this.string, this.start, this.tabSize, this.lastColumnPos, this
      .lastColumnValue );
...
StringStream.prototype.match = function( pattern, consume, caseInsensitive ) {
  if ( typeof pattern === 'string' ) {
    ...
    const match = this.string.slice( this.pos )
      .match( pattern );
...
function clipPos( doc, pos ) {
  ...
  if ( pos.line > last ) {
    return Pos( last, getLine( doc, last )
      .text.length );
  }
...
function getContextBefore( cm, n, precise ) {
  ...
  const saved = start > doc.first && getLine( doc, start - 1 )
    .stateAfter;
  ...
  doc.iter( start, n, ( line ) => {
    ...
    line.stateAfter = pos == n - 1 || pos % 5 == 0 || pos >= display.viewFrom && pos < display.viewTo ? context
      .save() : null;
    context.nextLine();
  } );
  if ( precise ) {
...
function findStartLine( cm, n, precise ) {
  ...
  for ( let search = n; search > lim; --search ) {
    ...
    if ( after && ( !precise || search + ( after instanceof SavedContext ? after.lookAhead : 0 ) <= doc
        .modeFrontier ) ) {
      return search;
    }
...
function stretchSpansOverChange( doc, change ) {
  ...
  const oldFirst = isLine( doc, change.from.line ) && getLine( doc, change.from.line )
    .markedSpans;
  const oldLast = isLine( doc, change.to.line ) && getLine( doc, change.to.line )
    .markedSpans;
  if ( !oldFirst && !oldLast ) {
    return null;
  }
    ...
    if ( gap > 0 && first ) {
      for ( let i$2 = 0; i$2 < first.length; ++i$2 ) {
        if ( first[ i$2 ].to == null ) {
          ...
          .push( new MarkedSpan( first[ i$2 ].marker, null, null ) );
        }
      }
    }
...
function conflictingCollapsedRange( doc, lineNo$$1, from, to, marker ) {
  ...
  if ( sps ) {
    for ( let i = 0; i < sps.length; ++i ) {
      ...
      if ( fromCmp <= 0 && ( sp.marker.inclusiveRight && marker.inclusiveLeft ? cmp( found.to, from ) >= 0 : cmp(
          found.to, from,
        ) > 0 ) ||
        fromCmp >= 0 && ( sp.marker.inclusiveRight && marker.inclusiveLeft ? cmp( found.from, to ) <= 0 : cmp( found
          .from, to ) < 0 ) ) {
        return true;
      }
    }
  ...
  for ( ;; ) {
    if ( nextChange == pos ) { // Update current marker set
      ...
      for ( let j = 0; j < spans.length; ++j ) {
        ...
        } else if ( sp.from <= pos && ( sp.to == null || sp.to > pos || m.collapsed && sp.to == pos && sp.from ==
            pos ) ) {
...
function updateLineClasses( cm, lineView ) {
  ...
  if ( lineView.line.wrapClass ) {
    ensureLineWrapped( lineView )
      .className = lineView.line.wrapClass;
  } else if ( lineView.node != lineView.text ) {
    lineView.node.className = '';
  }
  const textClass = lineView.textClass ? lineView.textClass + ' ' + ( lineView.line.textClass || '' ) : lineView.line
    .textClass;
...
function updateLineGutter( cm, lineView, lineN, dims ) {
  ...
  if ( lineView.line.gutterClass ) {
    ...
    lineView.gutterBackground = elt( 'div', null, 'CodeMirror-gutter-background ' + lineView.line.gutterClass,
      ( 'left: ' + ( cm.options.fixedGutter ? dims.fixedPos : -dims.gutterTotalWidth ) + 'px; width: ' + ( dims
        .gutterTotalWidth ) + 'px' ) );
  ...
  if ( cm.options.lineNumbers || markers ) {
    ...
    const gutterWrap = lineView.gutter = elt( 'div', null, 'CodeMirror-gutter-wrapper', ( 'left: ' + ( cm.options
      .fixedGutter ? dims.fixedPos : -dims.gutterTotalWidth ) + 'px' ) );
    ...
    if ( cm.options.lineNumbers && ( !markers || !markers[ 'CodeMirror-linenumbers' ] ) ) {
      lineView.lineNumber = gutterWrap.appendChild(
        elt( 'div', lineNumberFor( cm.options, lineN ),
          ...
          ( 'left: ' + ( dims.gutterLeft[ 'CodeMirror-linenumbers' ] ) + 'px; width: ' + ( cm.display
...
function positionLineWidget( widget, node, lineView, dims ) {
  if ( widget.noHScroll ) {
    ( lineView.alignable || ( lineView.alignable = [] ) )
    .push( node );
  ...
  if ( node.nodeType == 3 ) { // If it is a text node, use a range to retrieve the coordinates.
    for ( let i$1 = 0; i$1 < 4; i$1++ ) { // Retry a maximum of 4 times when nonsense rectangles are returned
      ...
      if ( ie && ie_version < 9 && start == 0 && end == place.coverEnd - place.coverStart ) {
        ...
        rect = getUsefulRect( range( node, start, end )
          .getClientRects(), bias );
      }
    ...
    let rects;
    if ( cm.options.lineWrapping && ( rects = node.getClientRects() )
      .length > 1 ) {
...
function pageScrollX() {
  ...
  if ( chrome && android ) {
    return -( document.body.getBoundingClientRect()
      .left - parseInt( getComputedStyle( document.body )
        .marginLeft ) );
  }
...
function wrappedLineExtent( cm, lineObj, preparedMeasure, y ) {
  ...
  const begin = findFirst( ( ch ) => {
    return measureCharPrepared( cm, preparedMeasure, ch - 1 )
      .bottom <= y;
  }, end, 0 );
  end = findFirst( ( ch ) => {
...
function wrappedLineExtentChar( cm, lineObj, preparedMeasure, target ) {
  ...
  const targetTop = intoCoordSystem( cm, lineObj, measureCharPrepared( cm, preparedMeasure, target ), 'line' )
    .top;
...
function coordsCharInner( cm, lineObj, lineNo$$1, x, y ) {
  ...
  if ( order ) {
    const part = ( cm.options.lineWrapping ? coordsBidiPartWrapped : coordsBidiPart )( cm, lineObj, lineNo$$1,
      preparedMeasure, order, x, y );
    ltr = part.level != 1;
...
function coordsBidiPartWrapped( cm, lineObj, _lineNo, preparedMeasure, order, x, y ) {
  ...
  for ( let i = 0; i < order.length; i++ ) {
    ...
    const ltr = p.level != 1;
    const endX = measureCharPrepared( cm, preparedMeasure, ltr ? Math.min( end, p.to ) - 1 : Math.max( begin, p.from ) )
      .right;
...
function compensateForHScroll( display ) {
  return display.scroller.getBoundingClientRect()
    .left - display.sizer.getBoundingClientRect()
    .left;
}
...
function posFromMouse( cm, e, liberal, forRect ) {
  const display = cm.display;
  if ( !liberal && e_target( e )
    .getAttribute( 'cm-not-content' ) == 'true' ) {
    return null;
  }
  ...
  if ( forRect && coords.xRel == 1 && ( line = getLine( cm.doc, coords.line )
    ...
    coords = Pos( coords.line, Math.max( 0, Math.round( ( x - paddingH( cm.display )
...
function prepareSelection( cm, primary ) {
  ...
  for ( let i = 0; i < doc.sel.ranges.length; i++ ) {
    ...
    if ( range$$1.from()
      .line >= cm.display.viewTo || range$$1.to()
      .line < cm.display.viewFrom ) {
      continue;
    }
...
function drawSelectionRange( cm, range$$1, output ) {
  ...
  function add( left, top, width, bottom ) {
    ...
    fragment.appendChild( elt( 'div', null, 'CodeMirror-selected', ( 'position: absolute; left: ' + left +
      'px;\n                             top: ' + top + 'px; width: ' + ( width == null ? rightSide - left :
  ...
  function drawForLine( line, fromArg, toArg ) {
    ...
    function wrapX( pos, dir, side ) {
      ...
      const ch = side == 'after' ? extent.begin : extent.end - ( /\s/.test( lineObj.text.charAt( extent.end - 1 ) ) ?
        2 : 1 );
    ...
    iterateBidiSections( order, fromArg || 0, toArg == null ? lineLen : toArg, ( from, to, dir, i ) => {
      ...
      if ( toPos.top - fromPos.top <= 3 ) { // Single line
        ...
        const left = openLeft ? leftSide : ( ltr ? fromPos : toPos )
          .left;
        const right = openRight ? rightSide : ( ltr ? toPos : fromPos )
          .right;
  ...
  const sFrom = range$$1.from();
  const sTo = range$$1.to();
  if ( sFrom.line == sTo.line ) {
    ...
    const leftEnd = drawForLine( sFrom.line, sFrom.ch, singleVLine ? fromLine.text.length + 1 : null )
      .end;
  ...
  for ( let i = 0; i < display.view.length; i++ ) {
    ...
    if ( ie && ie_version < 8 ) {
      ...
      if ( !wrapping && cur.text.firstChild ) {
        width = cur.text.firstChild.getBoundingClientRect()
          .right - box.left - 1;
      }
    }
...
function maybeScrollWindow( cm, rect ) {
  ...
  if ( doScroll != null && !phantom ) {
    const scrollNode = elt( 'div', '\u200b', null, ( 'position: absolute;\n                         top: ' + ( rect
      ...
      .bottom - rect.top + scrollGap( cm ) + display.barHeight ) + 'px;\n                         left: ' + (
      rect.left ) + 'px; width: ' + ( Math.max( 2, rect.right - rect.left ) ) + 'px;' ) );
...
function resolveScrollToPos( cm ) {
  ...
  if ( range$$1 ) {
    ...
    const from = estimateCoords( cm, range$$1.from );
    const to = estimateCoords( cm, range$$1.to );
...
const NativeScrollbars = function( place, scroll, cm ) {
  ...
  const horiz = this.horiz = elt( 'div', [ elt( 'div', null, null, 'height: 100%; min-height: 1px' ) ],
...
NativeScrollbars.prototype.update = function( measure ) {
  ...
  if ( needsH ) {
    ...
    this.horiz.firstChild.style.width = Math.max( 0, measure.scrollWidth - measure.clientWidth + totalWidth ) +
      'px';
  } else {
...
function endOperation_R2( op ) {
  ...
  if ( display.maxLineChanged && !cm.options.lineWrapping ) {
    op.adjustWidthTo = measureChar( cm, display.maxLine, display.maxLine.text.length )
      .left + 3;
...
function highlightWorker( cm ) {
  ...
  doc.iter( context.line, Math.min( doc.first + doc.size, cm.display.viewTo + 500 ), ( line ) => {
    if ( context.line >= cm.display.viewFrom ) { // Visible
      ...
      const resetState = line.text.length > cm.options.maxHighlightLength ? copyState( doc.mode, context.state ) :
        null;
      ...
      let ischange = !oldStyles || oldStyles.length != line.styles.length ||
        oldCls != newCls && ( !oldCls || !newCls || oldCls.bgClass != newCls.bgClass || oldCls.textClass != newCls
          .textClass );
...
function restoreSelection( snapshot ) {
  ...
  if ( snapshot.anchorNode && contains( document.body, snapshot.anchorNode ) && contains( document.body, snapshot
      .focusNode ) ) {
    const sel = window.getSelection();
...
function lastChangeEvent( hist, force ) {
  ...
  if ( hist.done.length && !lst( hist.done )
    .ranges ) {
    return lst( hist.done );
  }
...
function addChangeToHistory( doc, change, selAfter, opId ) {
  ...
  if ( ( hist.lastOp == opId ||
      ...
      ( ( change.origin.charAt( 0 ) == '+' && hist.lastModTime > time - ( doc.cm ? doc.cm.options.historyEventDelay :
          500 ) ) ||
    ...
    for ( let i = 0; i < line.markedSpans.length; ++i ) {
      ...
      if ( ( sp.from == null || ( preventCursorLeft ? sp.from <= pos.ch : sp.from < pos.ch ) ) &&
        ...
        if ( oldPos ) {
          ...
          if ( near && near.line == pos.line && ( diff = cmp( near, oldPos ) ) && ( dir < 0 ? diff < 0 : diff >
              0 ) ) {
...
function movePos( doc, pos, dir, line ) {
  ...
  if ( dir > 0 && pos.ch == ( line || getLine( doc, pos.line ) )
    .text.length ) {
...
function makeChangeSingleDocInEditor( cm, change, spans ) {
  ...
  if ( changeHandler || changesHandler ) {
    ...
    if ( changesHandler ) {
      ( cm.curOp.changeObjs || ( cm.curOp.changeObjs = [] ) )
      .push( obj );
    }
  }
...
Doc.prototype = createObj( BranchChunk.prototype, {
  ...
  getCursor( start ) {
    const range$$1 = this.sel.primary();
    let pos;
  ...
  setCursor: docMethodOp( function( line, ch, options ) {
    setSimpleSelection( this, clipPos( this, typeof line === 'number' ? Pos( line, ch || 0 ) : line ), null,
      options );
  } ),
  ...
  findMarksAt( pos ) {
    pos = clipPos( this, pos );
    const markers = [];
    const spans = getLine( this, pos.line )
      .markedSpans;
    if ( spans ) {
      for ( let i = 0; i < spans.length; ++i ) {
        const span = spans[ i ];
  ...
  posFromIndex( off ) {
    let ch;
    let lineNo$$1 = this.first;
    const sepSize = this.lineSeparator()
      .length;
    this.iter( ( line ) => {
  ...
  linkedDoc( options ) {
    ...
    const copy = new Doc( getLines( this, from, to ), options.mode || this.modeOption, from, this.lineSep, this
      .direction );
...
function deleteNearSelection( cm, compute ) {
  ...
  for ( let i = 0; i < ranges.length; i++ ) {
    ...
    while ( kill.length && cmp( toKill.from, lst( kill )
        .to ) <= 0 ) {
  ...
  if ( visually ) {
    ...
    if ( order ) {
      ...
      if ( part.level > 0 || cm.doc.direction == 'rtl' ) {
        ...
        const targetTop = measureCharPrepared( cm, prep, ch )
          .top;
        ch = findFirst( ( ch ) => {
          return measureCharPrepared( cm, prep, ch )
            .top == targetTop;
...
function moveVisually( cm, line, start, dir ) {
  ...
  if ( cm.doc.direction == 'rtl' || part.level == 1 ) {
    ...
    if ( ch != null && ( !moveInStorageOrder ? ch >= part.from && ch >= wrappedLineExtent.begin : ch <= part.to &&
...
const commands = {
  ...
  killLine( cm ) {
    return deleteNearSelection( cm, ( range ) => {
      if ( range.empty() ) {
        var len = getLine( cm.doc, range.head.line )
          .text.length;
  ...
  deleteLine( cm ) {
    return deleteNearSelection( cm, ( range ) => {
      return ( {
        from: Pos( range.from()
          .line, 0 ),
        to: clipPos( cm.doc, Pos( range.to()
          .line + 1, 0 ) ),
      } );
    } );
  },
  ...
  delWrappedLineLeft( cm ) {
    return deleteNearSelection( cm, ( range ) => {
      const top = cm.charCoords( range.head, 'div' )
        .top + 5;
  ...
  goLineRight( cm ) {
    return cm.extendSelectionsBy( ( range ) => {
      const top = cm.cursorCoords( range.head, 'div' )
        .top + 5;
  ...
  transposeChars( cm ) {
    return runInOp( cm, () => {
      ...
      for ( let i = 0; i < ranges.length; i++ ) {
        ...
        let line = getLine( cm.doc, cur.line )
          .text;
        if ( line ) {
          ...
          if ( cur.ch > 0 ) {
            ...
            const prev = getLine( cm.doc, cur.line - 1 )
              .text;
            if ( prev ) {
...
function leftButtonDown( cm, pos, repeat, event ) {
  ...
  if ( cm.options.dragDrop && dragAndDrop && !cm.isReadOnly() &&
    ...
    ( cmp( ( contained = sel.ranges[ contained ] )
      .from(), pos ) < 0 || pos.xRel > 0 ) &&
...
function leftButtonSelect( cm, event, start, behavior ) {
  ...
  if ( !behavior.addNew ) {
    ...
    setSelection( doc, normalizeSelection( cm, ranges.slice( 0, ourIndex )
      .concat( ranges.slice( ourIndex + 1 ) ), 0 ), {
      scroll: false,
      origin: '*mouse',
    } );
    startSel = doc.sel;
  ...
  function extendTo( pos ) {
    ...
    if ( behavior.unit == 'rectangle' ) {
      const ranges = [];
      const tabSize = cm.options.tabSize;
      const startCol = countColumn( getLine( doc, start.line )
        .text, start.ch, tabSize );
      const posCol = countColumn( getLine( doc, pos.line )
        .text, pos.ch, tabSize );
      const left = Math.min( startCol, posCol );
      const right = Math.max( startCol, posCol );
      for ( let line = Math.min( start.line, pos.line ), end = Math.min( cm.lastLine(), Math.max( start.line, pos
          .line ) ); line <= end; line++ ) {
      ...
      setSelection( doc, normalizeSelection( cm, startSel.ranges.slice( 0, ourIndex )
...
function bidiSimplify( cm, range$$1 ) {
  ...
  return anchor.ch == ch && anchor.sticky == sticky ? range$$1 : new Range( new Pos( anchor.line, ch, sticky ),
    head );
}
...
function gutterEvent( cm, e, type, prevent ) {
  ...
  if ( mX >= Math.floor( cm.display.gutters.getBoundingClientRect()
      .right ) ) {
    return false;
  }
  if ( prevent ) {
...
function defineOptions( CodeMirror ) {
  ...
  option( 'spellcheck', false, ( cm, val ) => {
    return cm.getInputField()
      .spellcheck = val;
  }, true );
  option( 'autocorrect', false, ( cm, val ) => {
    return cm.getInputField()
      .autocorrect = val;
  }, true );
  option( 'autocapitalize', false, ( cm, val ) => {
    return cm.getInputField()
      .autocapitalize = val;
  }, true );
  ...
  option( 'tabindex', null, ( cm, val ) => {
    return cm.display.input.getField()
      .tabIndex = val || '';
  } );
...
function indentLine( cm, n, how, aggressive ) {
  ...
  if ( how == 'prev' ) {
    if ( n > doc.first ) {
      indentation = countColumn( getLine( doc, n - 1 )
        .text, null, tabSize );
    } else {
      indentation = 0;
    }
    ...
    if ( range$$1.empty() ) {
      ...
      } else if ( cm.state.overwrite && !paste ) // Handle overwrite
      {
        to = Pos( to.line, Math.min( getLine( doc, to.line )
          .text.length, to.ch + lst( textLines )
          .length ) );
...
function triggerElectric( cm, inserted ) {
  ...
  for ( let i = sel.ranges.length - 1; i >= 0; i-- ) {
    ...
    if ( mode.electricChars ) {
      ...
      if ( mode.electricInput.test( getLine( cm.doc, range$$1.head.line )
          .text.slice( 0, range$$1.head.ch ) ) ) {
        indented = indentLine( cm, range$$1.head.line, 'smart' );
      }
    }
    if ( indented ) {
...
function hiddenTextarea() {
  const te = elt( 'textarea', null, null,
...
function addEditorMethods( CodeMirror ) {
  ...
  CodeMirror.prototype = {
    ...
    indentSelection: methodOp( function( how ) {
      ...
      for ( let i = 0; i < ranges.length; i++ ) {
        ...
        if ( !range$$1.empty() ) {
          ...
          if ( from.ch == 0 && ranges.length == newRanges.length && newRanges[ i ].from()
            .ch > 0 ) {
    ...
    getStateAfter( line, precise ) {
      ...
      return getContextBefore( this, line + 1, precise )
        .state;
    },

    cursorCoords( start, mode ) {
      let pos;
      const range$$1 = this.doc.sel.primary();
      if ( start == null ) {
        pos = range$$1.head;
    ...
    moveV: methodOp( function( dir, unit ) {
      ...
      doc.extendSelectionsBy( ( range$$1 ) => {
        ...
        if ( unit == 'page' && range$$1 == doc.sel.primary() ) {
          addToScrollTop( this$1, charCoords( this$1, pos, 'div' )
            .top - headPos.top );
        }
        return pos;
      }, sel_move );
    ...
    phrase( phraseText ) {
      ...
      return phrases && Object.prototype.hasOwnProperty.call( phrases, phraseText ) ? phrases[ phraseText ] :
        phraseText;
    },
  ...
  CodeMirror.registerHelper = function( type, name, value ) {
    ...
    console.log(type, name, value, helpers[ type ][ name ]);
  };
...
function findPosV( cm, pos, dir, unit ) {
  ...
  let y;
  if ( unit == 'page' ) {
    const pageSize = Math.min( cm.display.wrapper.clientHeight, window.innerHeight || document.documentElement
      .clientHeight );
...
ContentEditableInput.prototype.showPrimarySelection = function() {
  ...
  if ( cm.display.viewTo == cm.display.viewFrom || from.line >= cm.display.viewTo || to.line < cm.display
    .viewFrom ) {
    sel.removeAllRanges();
    return;
  }
...
ContentEditableInput.prototype.pollContent = function() {
  ...
  if ( from.ch == 0 && from.line > cm.firstLine() ) {
    from = Pos( from.line - 1, getLine( cm.doc, from.line - 1 )
      .length );
  }
  if ( to.ch == getLine( cm.doc, to.line )
    .text.length && to.line < cm.lastLine() ) {
    to = Pos( to.line + 1, 0 );
  }
  ...
  const oldText = getBetween( cm.doc, Pos( fromLine, 0 ), Pos( toLine, getLine( cm.doc, toLine )
    .text.length ) );
  ...
  const chTo = Pos( toLine, oldText.length ? lst( oldText )
    .length - cutEnd : 0 );
...
ContentEditableInput.prototype.onKeyPress = function( e ) {
  ...
  if ( !this.cm.isReadOnly() ) {
    operation( this.cm, applyTextInput )( this.cm, String.fromCharCode( e.charCode == null ? e.keyCode : e
      .charCode ), 0 );
  }
};
    ...
    if ( node.nodeType == 1 ) {
      ...
      if ( markerID ) {
        ...
        if ( found.length && ( range$$1 = found[ 0 ].find( 0 ) ) ) {
          addText( getBetween( cm.doc, range$$1.from, range$$1.to )
            .join( lineSep ) );
        }
        return;
      }
...
function locateNodeInLineView( lineView, node, offset ) {
  ...
  for ( let after = topNode.nextSibling, dist = textNode ? textNode.nodeValue.length - offset : 0; after; after = after
    .nextSibling ) {
...
TextareaInput.prototype.prepareSelection = function() {
  ...
  if ( cm.options.moveInputWithCursor ) {
    const headPos = cursorCoords( cm, doc.sel.primary()
      .head, 'div' );
...
TextareaInput.prototype.onContextMenu = function( e ) {
  ...
  te.style.cssText = 'position: absolute; width: 30px; height: 30px;\n      top: ' + ( e.clientY - wrapperBox.top -
      5 ) + 'px; left: ' + ( e.clientX - wrapperBox.left - 5 ) + 'px;\n      z-index: 1000; background: ' + ( ie ?
