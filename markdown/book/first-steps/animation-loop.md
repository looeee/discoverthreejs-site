...
Here's a better idea: we'll define the logic for updating each object _on the object itself_. Each object will expose that logic using a generic `.tick` method of its own. Now, the `Loop.tick` method will be simple. For each frame, we'll loop over a list of animated objects and tell each of them to `.tick` forward by one frame. It will look something like this:
